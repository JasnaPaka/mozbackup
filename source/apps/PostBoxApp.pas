...
type TPostBoxApp = class(TMozApp)
  private
    // TODO: temporary
    Registr: TRegistry;
  protected
...
procedure TPostBoxApp.searchApplication();
begin
  ...
  SearchDirectory (Registr, 'SOFTWARE\PostBox\PostBox\'+ Programy[11].Verze + '\Main', 'Install Directory', 2);

  if Programy[11].Nalezen then
    begin
      found:= true;
      version:= Programy[11].Verze;
      pathToApplication:= Programy[11].Cesta;
    end
  else
    begin
      found:= false;
...
begin
  ...
  stringList.Add('[AppData]\PostBox');
  profilesLocations:= stringList;

  stringList:= TStringList.Create;
  stringList.Add('postbox.exe');
  exeNames:= stringList;
end;
